...
from app.enums import DataStatusEnum, ApiCaseSuiteTypeEnum, CaseStatusEnum, SendReportTypeEnum, ReceiveTypeEnum, \
...
class BaseModel(db.Model, JsonUtil):
    ...
    def make_pagination(cls, form, get_filed: list = [], order_by=None, **kwargs):
        ...
        if form.page_num and form.page_size:
            ...
            query_result = query_obj.paginate(form.page_num, per_page=form.page_size, error_out=False)
            return {
...
class StatusFiled(BaseModel):
    __abstract__ = True
    status = db.Column(
        db.Integer(), default=DataStatusEnum.ENABLE.value, comment="数据状态，0/1")


class ScriptListFiled(BaseModel):
    __abstract__ = True
    script_list = db.Column(db.JSON, nullable=True, default=[], comment="引用的脚本文件")


class VariablesFiled(BaseModel):
    __abstract__ = True
    variables = db.Column(
        db.JSON, default=[{"key": None, "value": None, "remark": None, "data_type": None}], comment="公共参数"
    )


class HeadersFiled(BaseModel):
    __abstract__ = True
    headers = db.Column(db.JSON, default=[{"key": None, "remark": None, "value": None}], comment="头部信息")


class ParamsFiled(BaseModel):
    __abstract__ = True
    params = db.Column(db.JSON, default=[{"key": None, "value": None}], comment="url参数")


class BodyTypeFiled(BaseModel):
    __abstract__ = True
    body_type = db.Column(
        db.Enum(*ApiBodyTypeEnum.get_value_tuple()),
        default=ApiBodyTypeEnum.JSON,
...
class DataFormFiled(BaseModel):
    __abstract__ = True
    data_form = db.Column(
        db.JSON, comment="form-data参数", default=[{"data_type": None, "key": None, "remark": None, "value": None}])


class DataUrlencodedFiled(BaseModel):
    __abstract__ = True
    data_urlencoded = db.Column(db.JSON, default={}, comment="form_urlencoded参数")


class DataJsonFiled(BaseModel):
    __abstract__ = True
    data_json = db.Column(db.JSON, default={}, comment="json参数")


class ExtractsFiled(BaseModel):
    __abstract__ = True
    extracts = db.Column(
        db.JSON, comment="提取信息",
...
class UpFuncFiled(BaseModel):
    __abstract__ = True
    up_func = db.Column(db.JSON, default=[], comment="执行前置的函数")


class DownFuncFiled(BaseModel):
    __abstract__ = True
    down_func = db.Column(db.JSON, default=[], comment="执行后置的函数")


class BaseProject(ScriptListFiled):
    """ 服务基类表 """
    __abstract__ = True

    name = db.Column(db.String(255), unique=True, nullable=False, comment="服务名称")
    manager = db.Column(db.Integer(), nullable=False, comment="服务/项目/app管理员")
...
class BaseProjectEnv(VariablesFiled):
    ...
    host = db.Column(db.String(255), default=_main_server_host, comment="服务地址")
...
class BaseCaseSuite(BaseModel):
    ...
    suite_type = db.Column(
        db.Enum(*ApiCaseSuiteTypeEnum.get_value_tuple()), default=ApiCaseSuiteTypeEnum.BASE.value,
...
class BaseCase(ScriptListFiled, VariablesFiled, SkipIfFiled):
    """ 用例基类表 """
    ...
    status = db.Column(
        db.Integer(), default=CaseStatusEnum.NOT_DEBUG_AND_NOT_RUN.value,
    ...
    output = db.Column(db.JSON, default=[], comment="用例出参（步骤提取的数据）")
...
class BaseStep(StatusFiled, SkipIfFiled, UpFuncFiled, DownFuncFiled):
    """ 测试步骤基类表 """
    ...
    data_driver = db.Column(db.JSON, default=[], comment="数据驱动，若此字段有值，则走数据驱动的解析")
    quote_case = db.Column(db.String(5), default=None, comment="引用用例的id")
...
class BaseTask(StatusFiled):
    """ 定时任务基类表 """
    __abstract__ = True

    num = db.Column(db.Integer(), comment="任务序号")
    name = db.Column(db.String(255), comment="任务名称")
    env_list = db.Column(db.JSON, default=[], comment="运行环境")
    case_ids = db.Column(db.JSON, default=[], comment="用例id")
    task_type = db.Column(db.String(255), default="cron", comment="定时类型")
    cron = db.Column(db.String(255), nullable=True, comment="cron表达式")
    is_send = db.Column(
        db.Enum(*SendReportTypeEnum.get_value_tuple()), default=SendReportTypeEnum.NOT_SEND,
        comment="是否发送报告，1.不发送、2.始终发送、3.仅用例不通过时发送")
    receive_type = db.Column(
        db.Enum(*ReceiveTypeEnum.get_value_tuple()), default=ReceiveTypeEnum.DING_DING,
        comment="接收测试报告类型: ding_ding、we_chat、email")
    webhook_list = db.Column(db.JSON, default=[], comment="机器人地址")
    ...
    email_to = db.Column(db.JSON, default=[], comment="收件人邮箱")
    skip_holiday = db.Column(db.Boolean(), default=True, comment="是否跳过节假日、调休日")
    is_async = db.Column(db.Integer(), default=1, comment="任务的运行机制，0：串行，1：并行，默认0")
    suite_ids = db.Column(db.JSON, default=[], comment="用例集id")
    call_back = db.Column(db.JSON, default=[], comment="回调给流水线")
    project_id = db.Column(db.Integer(), index=True, comment="所属的服务id")
    conf = db.Column(
        db.JSON,
...
class BaseReport(BaseModel):
    ...
    temp_variables = db.Column(db.JSON, default={}, comment="临时参数")
    process = db.Column(db.Integer(), default=1, comment="进度节点, 1: 解析数据、2: 执行测试、3: 写入报告")
    trigger_type = db.Column(
        db.Enum(*TriggerTypeEnum.get_value_tuple()), default=TriggerTypeEnum.PAGE,
        comment="触发类型，pipeline:流水线、page:页面、cron:定时任务")
    batch_id = db.Column(db.String(128), index=True, comment="运行批次id，用于查询报告")
    trigger_id = db.Column(db.JSON, comment="运行id，用于触发重跑")
    project_id = db.Column(db.Integer(), index=True, comment="所属的服务id")
    summary = db.Column(db.JSON, default={}, comment="报告的统计")
...
class BaseReportCase(BaseModel):
    ...
    case_id = db.Column(db.Integer(), index=True, comment="执行记录对应的用例id")
    ...
    case_data = db.Column(db.JSON, default={}, comment="用例的数据")
    summary = db.Column(db.JSON, default={}, comment="用例的报告统计")
...
class BaseReportStep(BaseModel):
    ...
    element_id = db.Column(db.Integer(), comment="步骤对应的元素/接口id")
    ...
    result = db.Column(db.String(128), default='waite',
                       comment="步骤测试结果，waite：等待执行、running：执行中、fail：执行不通过、success：执行通过、skip：跳过、error：报错")
    step_data = db.Column(db.JSON, default={}, comment="步骤的数据")
    summary = db.Column(db.JSON,
                        default={"response_time_ms": 0, "elapsed_ms": 0, "content_size": 0, "request_at": "",
                                 "response_at": ""},
                        comment="步骤的统计")
