...
class Func(BaseModel):
    ...
    def create_func_file(cls, env='test'):
        """ 创建所有自定义函数 py 文件，默认在第一行加上运行环境，运行还款默认为test
        示例：
            # coding:utf-8
    ...
    @classmethod
    def get_func_by_func_file_name(cls, func_file_id_list):
        """ 获取指定函数文件中的函数 """
        cls.create_func_file()  # 创建所有函数文件
        func_dict = {}
        for func_file_id in func_file_id_list:
            func_list = importlib.reload(importlib.import_module(f'func_list.{Func.get_first(id=func_file_id).name}'))
            func_dict.update({
