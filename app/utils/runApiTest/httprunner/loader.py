...
import yaml
from . import exceptions, logger, utils, validator
...
def _check_format(file_path, content):
    ...
    if not content:
        ...
        err_msg = u"测试用例文件内容为空: {}".format(file_path)
        logger.log_error(err_msg)
        raise exceptions.FileFormatError(err_msg)

    elif not isinstance(content, (list, dict)):
        # testcase file content does not match testcase format
        err_msg = u"测试用例文件内容格式错误: {}".format(file_path)
        logger.log_error(err_msg)
        raise exceptions.FileFormatError(err_msg)
...
def load_json_file(json_file):
    """ 加载json文件，并检查json文件内容格式 """
    with io.open(json_file, encoding='utf-8') as data_file:
        try:
            json_content = json.load(data_file)
        except exceptions.JSONDecodeError:
            err_msg = u"json文件内容格式错误: {}".format(json_file)
            logger.log_error(err_msg)
            raise exceptions.FileFormatError(err_msg)
...
def load_csv_file(csv_file):
    """ 加载csv文件，并检查csv文件内容格式
    Args:
        csv_file (str): csv文件路径, csv文件内容如Examples所示:
    Returns:
        list: 参数列表，每个参数都是dict格式
...
def load_file(file_path):
    if not os.path.isfile(file_path):
        raise exceptions.FileNotFound("{} 不存在".format(file_path))
...
def load_folder_files(folder_path, recursive=True):
    """ 文件路径下以yml/yaml/json结尾的文件的list
    Args:
        folder_path (str): specified folder path to load
        recursive (bool): 如果为True，则递归加载文件
    Returns:
        list: 以yml/yaml/json结尾的文件list
    """
