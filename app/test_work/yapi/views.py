...
from .models import YapiProject, YapiModule, YapiApiMsg, YapiDiffRecord
...
def update_project(yapi_project):
    ...
    with db.auto_commit():
        ...
        test_work.logger.info(f'原始yapi服务信息: \n{yapi_project}')
...
def update_module(project, yapi_module):
    ...
    with db.auto_commit():
        ...
        test_work.logger.info(f'原始yapi模块信息: \n{yapi_module}')
    ...
    test_work.logger.info(f'解析yapi后的模块信息：\n{module.to_dict()}')
    return module
    ...
    with db.auto_commit():
        for api_index, yapi_api in enumerate(module_and_api.get('list', [])):
            if assert_coding_format(yapi_api['title']):
                test_work.logger.info(f'原始yapi接口信息: \n{yapi_api}')
                # 存原始信息
...
def get_group_list(host, headers, ignore_group):
    ...
    test_work.logger.info(f'获取到的分组列表：\n{group_list}')
...
def get_group(host, group_id, headers):
    ...
    test_work.logger.info(f'根据分组id {group_id} 获取到的分组：\n{group}')
...
def get_yapi_project_list(host, group_id, headers, ignore_project):
    ...
    test_work.logger.info(f'根据分组 {group_id} 获取到的服务：\n{project_list}')
...
def get_module_list(host, project_id, headers):
    ...
    test_work.logger.info(f'根据服务id {project_id} 获取到的分类：\n{res.get("data", {}).get("cat", [])}')
...
def get_module_and_api(host, project_id, headers):
    ...
    test_work.logger.info(f'根据服务id {project_id} 获取到的数据：\n{data}')
    return data
...
@test_work.route('/yapi/pull/all', methods=['POST'])
@login_required
def yapi_pull_all():
    """ 拉取yapi的所有数据
    id: 指定服务在测试平台的id
    ...
    for group in get_group_list(conf.get('yapi_host'), headers, conf.get('ignore_group')):
        ...
        for yapi_project in project_list:
            ...
            # 更新服务
            test_work.logger.info(f'服务：{yapi_project}')
            if assert_coding_format(yapi_project.get('name')):
                ...
                for yapi_module in get_module_list(conf.get('yapi_host'), yapi_project['_id'], headers):
                    test_work.logger.info(f'模块：{yapi_module}')
...
@test_work.route('/yapi/pull/project', methods=['POST'])
@login_required
...
@test_work.route('/yapi/diff/byApi', methods=['POST'])
@login_required
def diff_by_yapi():
...
@test_work.route('/project/diff/byFront', methods=['POST'])
@login_required
def diff_by_front():
...
@test_work.route('/yapi/diff/download', methods=['GET'])
@login_required
...
@test_work.route('/diffRecord/list')
...
@test_work.route('/diffRecord/project')
...
@test_work.route('/diffRecord/show')
