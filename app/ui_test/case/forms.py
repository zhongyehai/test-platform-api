...
class AddCaseForm(BaseForm):
    ...
    def validate_variables(self, field):
        ...
        env = UiProjectEnv.get_first(project_id=self.project.id, env=self.choice_host.data).to_dict()
    ...
    def validate_set_id(self, field):
        """ 校验用例集存在 """
        if not UiCaeSet.get_first(id=field.data):
            raise ValidationError(f'id为【{field.data}】的用例集不存在')

    def validate_name(self, field):
        """ 用例名不重复 """
        if UiCase.get_first(name=field.data, set_id=self.set_id.data):
...
class EditCaseForm(AddCaseForm):
    ...
    def validate_id(self, field):
        """ 校验用例id已存在 """
        old_data = UiCase.get_first(id=field.data)
        if not old_data:
    ...
    def validate_name(self, field):
        """ 同一用例集下用例名不重复 """
        old_data = UiCase.get_first(name=field.data, set_id=self.set_id.data)
...
class DeleteCaseForm(BaseForm):
    ...
    def validate_id(self, field):
        ...
        if not UiProject.is_can_delete(UiCaeSet.get_first(id=case.set_id).project_id, case):
