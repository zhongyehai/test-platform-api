...
class UiProject(BaseModel):
    ...
    def is_can_delete(cls, project_id, obj):
        ...
        3.当前用户为当前要删除服务的负责人
        """
        return UiProject.is_manager_id(project_id) or cls.is_admin() or obj.is_create_user(current_user.id)
    ...
    def make_pagination(cls, form):
        ...
        filters = []
        if form.name.data:
            filters.append(UiProject.name.like(f'%{form.name.data}%'))
        if form.projectId.data:
            filters.append(UiProject.id == form.projectId.data)
        if form.manager.data:
            filters.append(UiProject.manager == form.manager.data)
        if form.create_user.data:
            filters.append(UiProject.create_user == form.create_user.data)
    ...
    def create_env(self, env_list=None):
        ...
        for env in env_list:
            UiProjectEnv().create({"env": env, "project_id": self.id})


class UiProjectEnv(BaseModel):
    """ 服务环境表 """
    ...
    def to_dict(self, *args, **kwargs):
        ...
        return super(UiProjectEnv, self).to_dict(to_dict=["variables", "headers", "func_files"])
