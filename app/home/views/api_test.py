...
def home_get_api_test_api():
    ...
    method_query = Api.db.session.query(
        func.count(),
        func.count(case(whens=[
            (Api.method == 'GET', 'GET'), (Api.method == 'POST', 'POST'),
            (Api.method == 'PUT', 'PUT'), (Api.method == 'DELETE', 'DELETE')
        ], else_='Other'))
...
def home_get_api_test_report():
    ...
    run_type_query = Report.db.session.query(
        func.count(),
        func.count(case(whens=[
            (Report.run_type == 'api', 'api'), (Report.run_type == 'case', 'case'),
            (Report.run_type == 'suite', 'suite'), (Report.run_type == 'task', 'task')
        ], else_='Other'))
...
def home_get_api_test_hit():
    """ 统计命中数 """
    hit_type_data = Hits.db.execute_query_sql(f"""select hit_type, count(*) num from `{Hits.__tablename__}` group by hit_type""")
