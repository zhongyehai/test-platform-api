...
from flask import request, send_from_directory, current_app as app, views

from app.assist import assist
from utils.globalVariable import CASE_FILE_ADDRESS, CALL_BACK_ADDRESS, CFCA_FILE_ADDRESS, TEMP_FILE_ADDRESS, UI_CASE_FILE_ADDRESS
...
@assist.route('/file/create/download')
def creat_file_not_login_required():
    """ 创建指定大小和格式的文件 """
    start = time.time()
...
@assist.route('/file/check', methods=['GET'])
def check_file():
    """ 检查文件是否已存在 """
...
@assist.route('/file/download', methods=['GET'])
def download_file():
    """ 下载文件 """
    addr = folders.get(request.args.get('fileType'), 'case')
...
@assist.route('/upload', methods=['POST'], strict_slashes=False)
def file_upload():
    """ 文件上传 """
...
class FileManage(views.MethodView):
    """ 文件管理视图 """
...
assist.add_url_rule('/file', view_func=FileManage.as_view('file'))
