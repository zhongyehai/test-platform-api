...
def __parse_test_case_step(steps: list, case_config: dict, project_mapping: dict):
    """ 解析测试步骤
    ...
    if not case_config.get("browser_type", ''):  # 接口自动化
        ...
        for step_dict in steps:
            ...
            if step_dict.get("base_url"):
                # 解析 base_url
                base_url = parse_data(
                    step_dict.pop("base_url"),
                    step_dict["variables"],
                    functions
                )
...
def _parse_test_case(testcase, project_mapping):
    ...
    __parse_test_case_step(testcase["teststeps"], testcase["config"], project_mapping)
    ...
    for testcase_name, testcase in testcases.items():
        ...
        if "parameters" in testcase and testcase["parameters"]:
            ...
            for parameter_variables in cartesian_product_parameters:
                ...
                _parse_test_case(parsed_testcase_copied, project_mapping)
                parsed_testcase_list.append(parsed_testcase_copied)

        else:
            _parse_test_case(parsed_testcase, project_mapping)
