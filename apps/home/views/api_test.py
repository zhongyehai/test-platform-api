...
@home.login_get("/api-test/title-new")
def home_get_api_test_title_new():
    """ 获取卡片统计 """
    return app.restful.get_success([
        # {"name": "project", "title": "服务数", "total": Project.query.filter().count()},
        # {"name": "module", "title": "模块数", "total": Module.query.filter().count()},
        {"name": "api", "title": "接口数", "total": Api.query.filter().count()},
        # {"name": "hit", "title": "记录问题数", "total": Hits.query.filter().count()},
        {"name": "case", "title": "用例数", "total": Case.query.filter().count()},
        {"name": "step", "title": "测试步骤数", "total": Step.query.filter().count()},
        # {"name": "task", "title": "定时任务数", "total": Task.query.filter().count()},
